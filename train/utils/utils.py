...
from transformers.utils import PaddingStrategy, TensorType, to_py_obj
...
from transformers.tokenization_utils_base import PreTrainedTokenizerBase, BatchEncoding, is_tf_tensor, is_torch_tensor
...
def load_tokenizer_lc(data_args, model_args, training_args, logger):
    tokenizer_kwargs = {
        "cache_dir": model_args.cache_dir,
        "use_fast": model_args.use_fast_tokenizer,
        "revision": model_args.model_revision,
    ...
    class CustomTokenizer(TokenizerClass):
       
    
    tokenizer=CustomTokenizer.from_pretrained(model_args.model_name_or_path, **tokenizer_kwargs)
...
##lc_________________________________
def get_preprocessed_data(train_raw_data, valid_raw_data, test_raw_data, pairs, tokenizer, tokenizer1,shots_eval_dict, data_args, training_args, model_args):
    def tokenize_function_train_eval_left_pad(examples):
        ...
        return model_inputs
    def tokenize_function_train_left_pad(examples):
        inputs = []
        prompts = []
        for ex in examples["translation"]:
            source_lang, target_lang = list(ex.keys())
        ...
        for ex in examples["translation"]:
            source_lang, target_lang = list(ex.keys())
            if f"{source_lang}-{target_lang}" in pairs:
                prompt1 = get_prompt_few_shot(source_lang, target_lang, ex, shots_eval_dict)
                prompts1.append(prompt1)
                inputs1.append(prompt1 + ex[target_lang])
            if f"{target_lang}-{source_lang}" in pairs:
                prompt1 = get_prompt_few_shot(target_lang, source_lang, ex, shots_eval_dict)
                prompts1.append(prompt1)
                inputs1.append(prompt1 + ex[source_lang])
        model_inputs1 = tokenizer1(inputs1, max_length=data_args.max_source_length + data_args.max_new_tokens - 1, padding=padding, truncation=True, add_special_tokens=True)
        check_add_eos(model_inputs1, tokenizer1)
        labels1 = copy.deepcopy(model_inputs1)
        # If we are padding here, replace all tokenizer.pad_token_id in the labels by -100 when we want to ignore
        # padding in the loss.
        if padding == "max_length" and data_args.ignore_pad_token_for_loss:
            labels1["input_ids"] = [
                [(l if l != tokenizer1.pad_token_id else -100) for l in label1] for label1 in labels1["input_ids"]
            ]
            if data_args.ignore_prompt_token_for_loss:
                for idx, prompt1 in enumerate(prompts1):
                    prompt1 = tokenizer1(prompt1, max_length=data_args.max_source_length, add_special_tokens=False).input_ids
                    first_non_pad_idx = get_first_non_pad_index(labels1["input_ids"][idx])
                    labels1["input_ids"][idx][first_non_pad_idx: first_non_pad_idx + len(prompt1)] = [-100] * len(prompt1) 
        model_inputs1["labels"] = labels1["input_ids"]
        model_inputs["input_ids1"]=model_inputs1["input_ids"]
        model_inputs["attention_mask1"]=model_inputs1["attention_mask"]
        model_inputs["labels1"]=model_inputs1["labels"]
        return model_inputs
    ...
    mmt_train_tok_func = tokenize_function_train_eval_right_pad if data_args.right_pad else tokenize_function_train_left_pad
